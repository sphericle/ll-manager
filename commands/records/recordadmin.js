            ...
            subcommand
                ...
                .addUserOption((option) =>
                    option
                        .setName("moderator")
                        .setDescription(
                            "The moderator you want to check the activity of"
                        )
                        .setRequired(true)
                )
        )
    ...
    async execute(interaction) {
        ...
        if (interaction.options.getSubcommand() === "setstatus") {
            ...
            const update = await db.infos.update(
                {
                    status:
                        interaction.options.getString("status") === "closed",
                },
                { where: { name: "records" } }
            );
            ...
            logger.info(
                `Changed record status to ${interaction.options.getString(
                    "status"
                )}`
            );
            return await interaction.editReply(
                `:white_check_mark: Changed record status to ${interaction.options.getString(
                    "status"
                )}`
            );
            ...
            if (!modInfo) {
                return await interaction.editReply(
                    ":x: This moderator hasn't accepted or denied any record"
                );
            }
            ...
            const pendingChannel = await guild.channels.cache.get(
                pendingRecordsID
            );
            const priorityChannel = enablePriorityRole
                ? await guild.channels.cache.get(priorityRecordsID)
                : pendingChannel;
            for (let i = 0; i < pendingRecords.length; i++) {
                try {
                    if (enablePriorityRole && pendingRecords[i].priority)
                        await priorityChannel.messages.fetch(
                    ...
                    nbFound++;
                    logger.info(
                        `Found an errored record : ${pendingRecords[i].discordid}`
                    );
                    ...
                    try {
                        if (enablePriorityRole && pendingRecords[i].priority)
                            await (
                                await priorityChannel.messages.fetch(
                                    pendingRecordsID[i].embedDiscordid
                                )
                            ).delete();
                        else
                            await (
            ...
            return await interaction.editReply(
                `:white_check_mark: ${nbFound} records missing from the pending channel were found, and removed from the database.`
            );
        }
    },
};
