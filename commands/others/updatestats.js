    ...
    async execute(interaction) {
        ...
        for (const date of acceptedData) {
            if (
                !(await db.dailyStats.findOne({
                    where: { date: date.dataValues["date"] },
                }))
            )
                await db.dailyStats.create({
                    date: date.dataValues["date"],
                    nbRecordsAccepted: date.dataValues["count"],
                });
            else
        ...
        for (const date of deniedData) {
            if (
                ...
                await db.dailyStats.create({
                    date: date.dataValues["date"],
                    nbRecordsDenied: date.dataValues["count"],
                });
            else
                await db.dailyStats.update(
                    { nbRecordsDenied: date.dataValues["count"] },
        ...
        await interaction.editReply(
            ":white_check_mark: Successfully updated internal daily stats database"
        );
    },
};
