...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (interaction.options.getSubcommand() === "add") {
            ...
            if ((await db.shifts.count({ where: { moderator: mod.id } })) == 3)
                return await interaction.editReply(
                    ":x: This moderator already has 3 shifts assigned"
                );

            const addedShift = await db.shifts.create({
                moderator: mod.id,
                day: day,
            });
            if (addedShift)
                return await interaction.editReply(
                    `:white_check_mark: Successfully added ${day} shift for ${mod}`
                );
            else
                return await interaction.editReply(
                    `:x: Couldn't add ${day} shift for ${mod}`
                );
            ...
            if (
                (await db.shifts.count({
                    where: { moderator: mod.id, day: day },
                })) == 0
            )
                return await interaction.editReply(
                    ":x: This moderator does not have any assigned shift on that day"
                );
            ...
            if (
                ...
                return await interaction.editReply(
                    `:white_check_mark: Successfully removed ${day} shift for ${mod}`
                );
            else
                return interaction.editReply(
                    `:x: Couldn't remove ${day} shift for ${mod}, something went wrong`
                );
        } else if (interaction.options.getSubcommand() === "list") {
            if ((await db.shifts.count()) === 0)
                return await interaction.editReply(
                    ":x: There are no assigned shifts yet"
                );
            ...
            for (const shift of shifts)
                shiftsList[shift.day] += "<@" + shift.moderator + "> ";
            for (const shiftDay of Object.keys(shiftsList))
                shiftStr += `**${shiftDay}**: ${shiftsList[shiftDay]} \n`;
