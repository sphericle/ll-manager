...
module.exports = {
    ...
    async execute() {
        ...
        if (!changelogData)
            return logger.info("No changelog data for this day");
        ...
        for (const entry of changelogData) {
            if (entry.action === "placed")
                changelogText += `- **${entry.levelname}** was placed at #${
                    entry.new_position
                }, ${await getAboveBelow(entry)}.\n`;
            else if (entry.action === "raised")
                changelogText += `- **${entry.levelname}** was raised from #${
                    entry.old_position
                } to #${entry.new_position}, ${await getAboveBelow(entry)}.\n`;
            else if (entry.action === "lowered")
                changelogText += `- **${entry.levelname}** was lowered from #${
                    entry.old_position
                } to #${entry.new_position}, ${await getAboveBelow(entry)}.\n`;
