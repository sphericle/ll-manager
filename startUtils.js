...
module.exports = {
    ...
    async sequelizeInit(db, cache) {
        ...
        for (const table of Object.keys(cache)) {
            if (table !== "updateLevels" && table !== "updateUsers")
    ...
    async checkGithubPermissions(octokit) {
        logger.info(
            `Checking github token permissions for ${githubOwner}/${githubRepo}...`
        );
        try {
            ...
            if (data.permissions.push) {
                logger.info(
                    `Found push access to ${githubOwner}/${githubRepo}`
                );
            } else {
                logger.info(
                    `Couldn't find push access to ${githubOwner}/${githubRepo}`
                );
            }
        } catch (error) {
