...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const level = await db.levelsToLegacy.findOne({
        ...
        try {
            ...
            newCommit = await octokit.git.createCommit({
                ...
                message: `"Lowered " ${
                    level.filename
                } from ${currentPosition} to ${list.length + 1} (${
        ...
        const levelname = (
            await cache.levels.findOne({ where: { filename: level.filename } })
        )?.name;
        try {
        ...
        logger.info(
            `${interaction.user.tag} (${interaction.user.id}) moved ${level.filename} from ${currentPosition} to legacy (${list.length})`
        );
        try {
            logger.info(
