...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const level = await db.levelsFromLegacy.findOne({
        ...
        const legacy = JSON.parse(
            Buffer.from(legacy_response.data.content, "base64").toString(
                "utf-8"
            )
        );

        const currentLegacyPosition = legacy.indexOf(level.filename) + 1;
        if (currentLegacyPosition == -1)
            return await interaction.editReply(
                ":x: The given level is not on the legacy list"
            );
        ...
        try {
            ...
            newCommit = await octokit.git.createCommit({
                owner: githubOwner,
                repo: githubRepo,
                message: `Moved ${level.filename} from legacy (${
                    list.length + currentLegacyPosition
                }) to ${level.position} (${interaction.user.tag})`,
        ...
        logger.info(
            `${interaction.user.tag} (${interaction.user.id}) moved ${
                level.filename
            } from legacy (${list.length + currentLegacyPosition}) to (${
                level.position
            })`
        );
        try {
